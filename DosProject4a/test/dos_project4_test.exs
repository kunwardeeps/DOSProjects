defmodule KryptoCoin.Test do
  ...
  test "21. Modify transaction amount and check if blockchain is still valid" do
    ...
    IO.inspect(KryptoCoin.Block.get_digest(modified_block))
    IO.inspect(KryptoCoin.Block.get_hash(modified_block))
    modified_blockchain = List.replace_at(blockchain, 0, modified_block)
    IO.inspect(KryptoCoin.Block.get_digest(first_block))
    IO.inspect(KryptoCoin.Block.get_hash(first_block))
    assert KryptoCoin.Block.validate(block, modified_blockchain)
  end
