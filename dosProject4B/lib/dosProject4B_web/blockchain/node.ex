defmodule KryptoCoin.Node do
  ...
  @first_coinbase_amount 2000.0
  @coinbase_amount 1000.0
  ...
  def init(existing_node_pid) do
    ...
    if (existing_node_pid == nil) do
      coinbase = KryptoCoin.Transaction.generate_coinbase(@first_coinbase_amount, wallet.public_key, wallet.private_key)
  ...
  def handle_call({:receive_transaction, transaction}, _from, [blockchain, wallet, transaction_pool, utxos]) do
    ...
    print("Transaction received, result = #{result}")
    if (result == :ok) do
      utxos = update_utxos(transaction, utxos)
      transaction_pool = Map.put(transaction_pool, transaction.id, transaction)
  ...
  def handle_call({:receive_block, block}, _from, [blockchain, wallet, transaction_pool, utxos]) do
    ...
    print("Block received, result = #{result}")
    if (result == :ok) do
  ...
  def handle_call({:mine_block}, _from, [blockchain, wallet, transaction_pool, utxos]) do
    if (Enum.count(transaction_pool) == 0) do
      print("No transactions to mine in the pool!")
  ...
  def broadcast_block(block) do
    ...
    if (approval >= 0.51) do
      print("Block validated by #{inspect(approval * 100)}% peers!")
      :ok
    else
      print("Block failed to meet consensus by #{inspect(approval * 100)}% peers!")
      :block_disapproved
    end
  end
  ...
  def broadcast_transaction(transaction) do
    ...
    approval = count/Enum.count(nodes)
    if (approval >= 0.51) do
      print("Transaction validated by #{inspect(approval * 100)}% peers!")
      :ok
    else
      print("Transaction failed to meet consensus by #{inspect(approval * 100)}% peers!")
      :transaction_failed
    end
  end
  ...
  def get_first_coinbase_amount() do
    @first_coinbase_amount
  end
